...
function Login({}: Props) {
  ...
  const handleLogin = async (e: any) => {
    ...
    try {
      ...
      if (phoneNumber === String(123) && password === "admin") {
        router.push("/admin");
      } else if (localStorage.getItem("token") === response.data.token) {
        router.push("/");
      }
    } catch (error) {
      console.error(error);
    ...
    <section className="relative flex flex-wrap lg:h-screen lg:items-center">
      <div className="w-full px-4 py-12 sm:px-6 sm:py-16 lg:w-1/2 lg:px-8 lg:py-24">
        ...
        >
          ...
          <Alert
            message="Admin credentials - phoneNumber - 123, password - admin"
            type="info"
          />
        </form>
      </div>
